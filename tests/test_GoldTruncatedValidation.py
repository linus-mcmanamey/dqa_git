...
class DataLoader:
    def __call__(self, spark_session: SparkSession, table_name: str, data_source_name: str) -> LoadedData:
        source_sdf = utils.target_table_dataframe(spark_session, table_name, data_source_name)
        target_sdf = utils.truncated_table_dataframe(spark_session, table_name, data_source_name)
